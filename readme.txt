...
ERROR in THE END:
1. Добавить try Except а все страницы (switch ) в обработку форму в начале, что-то пошло не так
2. Добавить обрабочик форм ** (передаем словарь в функцию, а функция удаляет пробелы, приводит поля к стандартному виду)
3. Сделать чтобы адрес коммутатора по умолчанию был склад
4. При добавлении ipswitch нужно чтобы была проверка тот ли IP адрес мы даем
5. проработать запись текста исключений и ошибок в файл
6. Добавить справочники доступных сетей (а также их генераторы)
7. При удалении АДРЕСА необходимо удалять все устройства подвязанные с ним... ПОКА не УДАЛЯЕМ ничего...
8. ModelSwitch  в models большие буквы
9. После загрузки моделей коммутаторов выходит ошибка что то пошло не так... хотя в базу добавляются
10. После перехода с динамических районов (в узлах ) на вкладку Device озникает ошибка и адрес странный http://127.0.0.1:5000/addnode/devices а должен быть http://127.0.0.1:5000/devices
11. Для добавление Ip адресов коммуттору, должен формироваться список из vlan создержаших MNG и адреса узла (планируем давать IP адреса коммутаторам по районам)
12. # from modelisw ошибка при добавлении (хотя модели добавляет)
# Порядок работы с программой


- Добавление коммуаттора на адрес происходит во вкладке Узлы - Конкретный узел. Далее Мы выбираем из списка по ID (в списке выводятся коммутаторы ТИПА "ACCESS" и свободные, то есть ADDRESS_ID = NULL)
